...
// ../.wrangler/tmp/bundle-rUcpfT/checked-fetch.js
...
async function onRequest(context) {
  ...
  try {
      ...
      const clubsWithDetails = await Promise.all(
        clubs.results.map(async (club) => {
          const members = await env.DB.prepare(
            "SELECT id, name, role FROM club_members WHERE club_id = ?"
          ).bind(club.id).all();
    ...
    if (path.match(/^\/clubs\/\d+\/join$/) && method === "POST") {
      ...
      const existing = await env.DB.prepare(
        "SELECT id FROM club_members WHERE club_id = ? AND id = ?"
      ).bind(clubId, userId).first();
      if (existing) {
        return jsonResponse({ error: "Already a member of this club" }, 409);
      }
      await env.DB.prepare(
        "UPDATE club_members SET club_id = ? WHERE id = ?"
...
// ../.wrangler/tmp/pages-2OynUd/functionsRoutes-0.2782625845800033.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-rUcpfT/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-rUcpfT/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.34505443894118004.mjs.map
